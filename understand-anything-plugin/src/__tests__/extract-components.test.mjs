...
describe("extractFileNodes + buildOverlay (pipeline)", () => {
  const graph = {
    nodes: [
  ...
  it("maps assigned files back to node ids in the overlay", () => {
    const components = [comp({ id: "a", name: "A", owner: "me", globs: ["src/**"] })];
    const { filePaths, nodeIdsByPath } = extractFileNodes(graph);
    const mr = matchFilesToComponents(filePaths, components);
    const overlay = buildOverlay(mr, components, nodeIdsByPath, { generatedAt: "T" });
    expect(overlay.components[0]).toMatchObject({
      id: "a",
      owner: "me",
      parent: null,
    ...
    expect(overlay.stats.unassignedFiles).toBe(1); // cfg.json unowned
  });
});
