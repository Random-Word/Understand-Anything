 ...
 *
 *   1. PIPELINE INJECTION (default): reads the assembled knowledge graph and
 *      injects declared components AS DURABLE GRAPH NODES — one `module` node
 *      per component (tagged `ua:declared-component`) plus `contains` edges to
 *      its member file-bearing nodes, and `contains` edges for parent nesting.
 *      When a component declares a `spec_path` that was already scanned as a
 *      `document` node, `specifies` edges are seeded from the spec document to
 *      each member (provenance-marked so the step is idempotent). The mutated
 *      graph is written back in place; ownership drift is written to
 *      `components-drift.json`. The injection is scrub-then-rebuild: any prior
 *      declared-component nodes/edges and seeded `specifies` edges are removed
 *      first, so re-runs converge. (The `normative-spec` document tag is owned
 *      by the merge step, not seeded here.)
 *
 ...
 *
 * The file-list / injection / drift builders are exported as pure functions for tests.
 */
...
export function validateComponentsConfig(config) {
  ...
  for (let i = 0; i < config.components.length; i++) {
    ...
    if (c.spec_path !== undefined && c.spec_path !== null &&
        (typeof c.spec_path !== 'string' || c.spec_path.length === 0)) {
      errors.push(`${where}: "spec_path" must be a non-empty string when present`);
    }
  }
...
// ===========================================================================
// Graph injection (declared components become durable graph primitives)
//
// A declared component is materialized as a `module` node tagged
// `ua:declared-component` plus `contains` edges to its member file-bearing
// nodes. Parent nesting is a `contains` edge module(parent) -> module(child).
// When a component declares a `spec_path` that already exists as a scanned
// `document` node, `specifies` edges are seeded from the spec document to each
// member node. (The `normative-spec` document tag is applied by the merge step,
// not here.)
//
// Injection is scrub-then-rebuild so it is idempotent across re-runs and
// converges when components are added/removed/renamed: all prior
// declared-component nodes (and their incident edges) and all provenance-marked
// seeded `specifies` edges are removed before re-adding.
// ===========================================================================

export const DECLARED_COMPONENT_TAG = 'ua:declared-component';

/** Stable node id for a declared component's module node. */
export function moduleNodeId(componentId) {
  return `module:${componentId}`;
}

// Provenance marker embedded in a seeded `specifies` edge description. The
// trailing colon makes the scrub match unambiguous (`...:<componentId>`).
const SEED_MARKER = 'ua:declared-component';
function seededSpecifiesDescription(componentId) {
  return `Declared-component spec link (${SEED_MARKER}:${componentId}).`;
}
function isSeededSpecifies(edge) {
  return edge && edge.type === 'specifies' &&
    typeof edge.description === 'string' &&
    edge.description.includes(`${SEED_MARKER}:`);
}

/**
 * Remove all previously injected declared-component artifacts from a graph
 * IN PLACE: module nodes tagged `ua:declared-component`, every edge incident to
 * those nodes, and every provenance-marked seeded `specifies` edge. Idempotent.
 * @returns {{ removedNodeIds: Set<string> }}
 */
export function scrubDeclaredComponents(graph) {
  const nodes = Array.isArray(graph.nodes) ? graph.nodes : [];
  const edges = Array.isArray(graph.edges) ? graph.edges : [];
  const removedNodeIds = new Set();
  graph.nodes = nodes.filter((n) => {
    const declared = n && n.type === 'module' &&
      Array.isArray(n.tags) && n.tags.includes(DECLARED_COMPONENT_TAG);
    if (declared) removedNodeIds.add(n.id);
    return !declared;
  });
  graph.edges = edges.filter((e) => {
    if (!e) return false;
    if (removedNodeIds.has(e.source) || removedNodeIds.has(e.target)) return false;
    if (isSeededSpecifies(e)) return false;
    return true;
  });
...
/**
 * Inject declared components into a knowledge graph IN PLACE.
 * @param graph         the knowledge graph (mutated)
 * @param matchResult   result of matchFilesToComponents (file ownership)
 * @param components    declared components (id, name, globs, parent?, spec_path?)
 * @param nodeIdsByPath Map<filePath, nodeId[]> from extractFileNodes
 * @returns {{ stats, specFindings }}
 */
export function injectComponents(graph, matchResult, components, nodeIdsByPath) {
  ...
  const addEdge = (e) => {
    const k = edgeKey(e);
    if (existingEdgeKeys.has(k)) return false;
    existingEdgeKeys.add(k);
    graph.edges.push(e);
    return true;
  };

  // Index already-scanned document nodes by normalized path for spec_path linking.
  const docNodeByPath = new Map();
  for (const n of graph.nodes) {
    if (n && n.type === 'document' && typeof n.filePath === 'string') {
      const norm = n.filePath.replace(/\\/g, '/').replace(/^\.\//, '');
      if (!docNodeByPath.has(norm)) docNodeByPath.set(norm, n);
    }
  }
  ...
  // Pass 1: module nodes + member contains edges + spec_path seeding.
  for (const c of components) {
    const modId = moduleNodeId(c.id);
    if (!existingNodeIds.has(modId)) {
      graph.nodes.push({
        id: modId,
        type: 'module',
        name: c.name,
        summary: `Declared component "${c.name}".`,
        tags: [DECLARED_COMPONENT_TAG],
        complexity: 'moderate',
      });
      existingNodeIds.add(modId);
      modulesAdded++;
    }
    ...
    for (const fp of files) {
      for (const nid of (nodeIdsByPath.get(fp) || [])) {
        memberNodeIds.push(nid);
        if (addEdge({ source: modId, target: nid, type: 'contains', direction: 'forward', weight: 1.0 })) {
          memberEdges++;
        }
      }
    }
    ...
    if (c.spec_path) {
      const norm = String(c.spec_path).replace(/\\/g, '/').replace(/^\.\//, '');
      const doc = docNodeByPath.get(norm);
      if (!doc) {
        specFindings.push({
          type: 'spec_not_found',
          severity: 'drift',
          componentId: c.id,
          specPath: norm,
          message: `Component "${c.id}" spec_path "${norm}" has no scanned document node — spec link skipped.`,
        });
      } else {
        // NOTE: we deliberately do NOT tag the document `normative-spec` here.
        // That tag is owned by the merge step (`link_specs`), which applies it
        // deterministically from LLM-authored `specifies` edges at merge time.
        // Tagging here would (a) be un-scrubbable without provenance and (b)
        // risk clobbering merge-authored tags on re-runs. The seeded `specifies`
        // edge below is the durable, idempotently-scrubbed signal.
        for (const nid of memberNodeIds) {
          if (addEdge({
            ...
            direction: 'forward',
            weight: 0.6,
            description: seededSpecifiesDescription(c.id),
          })) {
            specEdges++;
          }
        }
  ...
  // Pass 2: parent nesting (both module nodes now exist).
  for (const c of components) {
    if (c.parent) {
      if (addEdge({
        source: moduleNodeId(c.parent),
        target: moduleNodeId(c.id),
        type: 'contains',
        direction: 'forward',
        weight: 1.0,
      })) {
...
async function main() {
  ...
  if (!existsSync(componentsPath)) {
    process.stdout.write('No .understand-anything/components.json — skipping component injection.\n');
    process.exit(0);
  }
...
async function runPipeline(opts, config, coverage, uaDir) {
  ...
  const { stats, specFindings } = injectComponents(graph, matchResult, config.components, nodeIdsByPath);
  const { findings } = buildDrift(matchResult, {
    coverage,
    severities: opts.severities,
    categoryOf: detectCategory,
  });
  const allFindings = [...findings, ...specFindings];
  const summary = { error: 0, drift: 0 };
  ...
  // Write the injected graph back in place so downstream validate/save steps
  // persist the declared-component nodes/edges. Drift is a separate report.
  ...
  for (const f of allFindings.filter((x) => x.severity === 'error')) {
    process.stderr.write(`Warning: component drift (${f.type}): ${f.message}\n`);
  }
  process.stdout.write(
    `Components: ${stats.declaredComponents} declared, ${stats.modulesAdded} module node(s), ` +
    `${stats.memberEdges} contains edge(s), ${stats.specEdges} specifies edge(s), ` +
    `${allFindings.length} drift (${summary.error || 0} error).\n`,
  );
  process.exit(0);
}
